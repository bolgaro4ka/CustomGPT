...
def validetor(question):
    ...
    if (
        ("погода" in question)
        or ("погоде" in question)
        or ("погоду" in question)
        or ("погодка" in question)
        or ("Погода" in question)
    ):
        try:
            params = {
                "q": "Novodugino,RU",
        ...
        del question[0]
        response = openai.Image.create(size=question[-1], prompt="".join(question[:-1]), n=1)
