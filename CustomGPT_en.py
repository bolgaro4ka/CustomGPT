...
root.title("CustomGPT v.2.2.1-GUI-EN-Release")
...
main_image = main_image.resize((int(width_photo // 4.2), int(height_photo // 4.2)))  ## The (250, 250) is (height, width)
...
gptmodel = ["gpt-3.5-turbo-16k", "gpt-3.5-turbo-16k-0613", "gpt-4-0613"]
...
def typing():
    ...
    st.insert(
        (str(count_message) + ".0"),
        os.getlogin() + ": " + user_answer_entry.get() + "\n\n",
    )
    count_message += 3
    if free.get():
...
author = customtkinter.CTkButton(
    root,
    text="By Bolgaro4ka",
    width=415,
    fg_color="black",
    text_color="white",
    image=gitlogo,
    command=lambda: webbrowser.open("https://github.com/bolgaro4ka"),
)
